        ...
        switch (v.getId())
        {
            case R.id.login_button:
                ...
                } else if (userPassword.getText().toString().trim().equals(""))
                {
                    ...
                    login.setText("登\t\t\t录");
                    login.setEnabled(true);
                } else if (userPassword.getText().toString().trim().length() < 6 || userPassword
                        .getText().toString().trim().length() > 16)
                {
                    Toast.makeText(LoginActivity.this, "密码为6-16位字符组成,不包含空字符串", Toast
                            .LENGTH_SHORT).show();
                    login.setText("登\t\t\t录");
                    login.setEnabled(true);
                } else
                {
                    ...
                    OkHttpUtils
                            ...
                            .execute(new StringCallback()
                            {
                                ...
                                {
                                    Toast.makeText(LoginActivity.this, "网络未连接请重试", Toast
                                            .LENGTH_SHORT).show();
                                    login.setText("登\t\t\t录");
                                    login.setEnabled(true);
                                }
                                ...
                                public void onResponse(String response)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            Toast.makeText(LoginActivity.this, "该账号不存在,请先注册",
                                        ...
                                        } else if (message.equals("ok"))
                                        {
                                            ...
                                            String imagePath = APIAddr.BASE_IMG_URL + data
                                                    .getString("user_icon");
